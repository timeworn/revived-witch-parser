...
import { ICAudioPlayerCellData, ItemAmount } from "../../../interfaces/CharacterInterfaces";
...
export class RWMusicBox extends RWAudio {
  ...
  constructor(audioCell: ICAudioPlayerCellData) {
    ...
    this.itemsNeeded = RWUtils.createItemAmount(audioCell.unlockItems, audioCell.itemsAmount);
  }

  static getMusic() {
    return RWUtils.getAudioPlayerCellData().map((audioCell) => new RWMusicBox(audioCell));
  }
  ...
  getAlbumName() {
    return RWTexts.getWordYard(RWUtils.getAudioPlayerAlbum(this.album).albumName) ?? "?";
  }
