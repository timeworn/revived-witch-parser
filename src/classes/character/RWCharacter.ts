...
export class RWCharacter {
  ...
  static getCharacters() {
    if (!characters) {
      characters = RWUtils.getConfigJson().AllIds.map((id: number) => {
  ...
  static getElements() {
    ...
    return RWUtils.getElementConfigJson().AllIds.map((id: number) => {
      const elementConfig = RWUtils.getElementConfig(id - 1) as ICharacterElementData;
  ...
  static getVocations() {
    ...
    return vocationJson.AllIds.map((id: number) => {
      const vocationConfig = RWUtils.getVocationConfig(id - 1) as ICharacterVocationData;
